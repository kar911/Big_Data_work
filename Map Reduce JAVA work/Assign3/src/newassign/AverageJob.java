...
//	public enum Counters {MAP, COMBINE, REDUCE}
	public enum Counters {GOODrecord,BADrecord}
...
//			outputValue.set(words[9] + ONE);
			if (words.length == 12){
				...
				context.getCounter(Counters.GOODrecord).increment(1);	
			}
			else{
				context.getCounter(Counters.BADrecord).increment(1);
			}
	...
	public static class AverageReducer extends Reducer<Text, Text, Text, DoubleWritable> {
		...
		protected void cleanup(Context context)
				throws IOException, InterruptedException {
			System.out.println("GOODrecord count = " + context.getCounter(Counters.GOODrecord).getValue());
			System.out.println("BADrecord count = " + context.getCounter(Counters.BADrecord).getValue());
